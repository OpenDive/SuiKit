...
internal struct KioskToolbox {
    ...
    func prepareHeroRuleset(heroPackageId: String) async throws {
        ...
        let _ = try await testToolbox.client.waitForTransaction(tx: tx_res.digest)
    }
    ...
    func purchaseFlow(buyerCap: KioskOwnerCap, sellerCap: KioskOwnerCap, itemType: String, itemId: String) async throws {
        ...
        let _ = try await self.testToolbox.client.waitForTransaction(tx: tx_res_sell.digest)
        ...
        let _ = try await self.testToolbox.client.waitForTransaction(tx: tx_res_purchase.digest)
    }
